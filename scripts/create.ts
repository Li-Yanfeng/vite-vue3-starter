...
const useDestDirPath = (type: string) => p(`../src/${type}s`)
...
const run = async () => {
    const genType = await useInquirerList('😋 您要创建以下哪种类型的文件?', {
        default: 'component',
        choices: ['page', 'component']
    })
    const type = await useInquirerList('🧐 请选择您要创建的类型', {
        default: 'vue',
        choices: ['md', 'vue']
    })

    const name = await useInquirerQuestion('🤔 请输入您要创建的名字')
    ...
    const shouldGens = await Promise.all([isWillCreate(mdDest, cTip), isWillCreate(vueDest, cTip)])
...
// 生成文件
const gen = async (src: string, dest: string, payload: any = {}) => {
